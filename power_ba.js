document.addEventListener("DOMContentLoaded", () => {
    ...
    function calculateStats() {
		...
		if (allStats.length === 0) return; // Ei laskettavaa

		let isValid = true;

		allStats.forEach(stats => {
			const inputs = stats.querySelectorAll("input[required]"); // Tarkistaa vain required-kentät
			inputs.forEach(input => {
				// Jos kenttä on tyhjä, lisää virheluokka ja aseta tapahtumankuuntelija
				if (!input.value.trim()) {
					input.classList.add("error");
					isValid = false;

					// Lisää tapahtumankuuntelija, joka poistaa virheluokan, kun kenttä täyttyy
